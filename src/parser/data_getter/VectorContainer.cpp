...
VectorContainer::VectorContainer() :
    AContainer(std::shared_ptr<IType>(new VectorType(0, 0, 0)), {}),
    _getX(false), _getY(false), _getZ(false) {}

void VectorContainer::execContainer(__attribute__((unused)) const nlohmann::json &json)
{

}

void VectorContainer::execContainer(nlohmann::json::const_iterator &jsonIterator)
{
    std::shared_ptr<VectorType> value(dynamic_cast<VectorType *>(_value.get()), [](VectorType *){});

    if (!jsonIterator->is_object()) {
        throw ContainerException("-E- >> Key 'vector' is not object.");
    }
